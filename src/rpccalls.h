...
#ifndef CROWSISPOP_RPCCALLS_H
#define CROWSISPOP_RPCCALLS_H
...
{
// declare struct in sispop's cryptonote namespace.
// sispop should provide definition for this,
...
public:
    ...
    rpccalls(string _daemon_url = "http:://127.0.0.1:30000",
             uint64_t _timeout = 200000);

    bool
    connect_to_sispop_daemon();
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Sispop daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Sispop daemon at "
                 << daemon_url << endl;
            return false;
        }
