...
bool
rpccalls::connect_to_sispop_daemon()
{
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_staking_requirement: error connecting to Sispop daemon at "
             << daemon_url << endl;
        return false;
    }
...
rpccalls::get_service_node(COMMAND_RPC_GET_SERVICE_NODES::response &res, const std::vector<std::string> &pubkeys)
{
    ...
    if (!result)
        cerr << "Error connecting to Sispop daemon at " << daemon_url << endl;
