...
#ifndef CROWSISPOP_PAGE_H
#define CROWSISPOP_PAGE_H



#include "mstch/mstch.hpp"

#include "sispop_headers.h"
...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        const double lok_amount = SISPOP_AMOUNT(fee);
...
void add_tx_metadata(mstch::map &context, const cryptonote::transaction &tx, bool detailed = false) {
        ...
        if (new_style || get_service_node_state_change_from_tx_extra(tx.extra, state_change, cryptonote::network_version_11_infinite_staking)) {
           if (new_style) context["state_change_new_style"] = true;
            ...
            if (detailed) {
                ...
                // recent, this requires sispopd to be started with --store-quorum-history (PR #702)
    ...
    else if (tx.version >= txversion::v4_tx_types && tx.type == txtype::sispop_name_system)
    {
      tx_extra_sispop_name_system lns_extra = {};
      if (get_sispop_name_system_from_tx_extra(tx.extra, lns_extra))
      {
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_sispop_network_info(j_info);
...
show_block(uint64_t _blk_height)
{
    ...
    // get sispop in the block reward
    context["blk_reward"]
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Sispop address not provided!");
    }
    ...
    // parse string representing given sispop address
    ...
    if (!lokeg::parse_str_address(lok_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse Sispop address: " + lok_address_str);
    }
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_sispop_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
search(string search_text)
{
    ...
    // check if sispop address is given based on its length
    ...
    // check if integrated sispop address is given based on its length
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Sispop address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Sispop address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_sispop_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Sispop network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"sispop_version_full"   , string {SISPOP_VERSION_FULL}},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = SISPOP_AMOUNT(txd.fee) / tx_size;
...
bool
get_sispop_network_info(json& j_info)
{
