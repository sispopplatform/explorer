# Sispop Onion Blockchain Explorer
 ...
 **Blocks and transactions**
 ...
 - shows public components of Sispop addresses,
 - decoding which outputs and mixins belong to the given Sispop address and viewkey,
 - can prove that you send Sispop to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - shows number of amount output indices,
 - support Sispop testnet, stagenet network,
...
Current development branch:
 ...
 - https://github.com/doy-lee/onion-sispop-blockchain-explorer/tree/sispop



## Compilation on Ubuntu 16.04/18.04

##### Compile latest Sispop master version

Download and compile recent Sispop into your home folder:

```bash
# first install Sispop dependecines
sudo apt update
...
Once the Sispop is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/sispop
cd ~

# download the source code
git clone https://github.com/sispop-project/sispop-onion-blockchain-explorer.git

# enter the downloaded sourced code folder
cd sispop-onion-blockchain-explorer
...
# alternatively can use: cmake -DSISPOP_DIR=/path/to/sispop ..
# if sispop is not in ~/sispop
#
...
By default it will look for blockchain in its default location i.e., `~/.sispop/lmdb`.
...
```
lokblocks, Onion Sispop Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Sispop total emission monitoring
                                        thread
...
alias lokblocksmainnet='~/sispop-onion-blockchain-explorer/build/lokblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias lokblockstestnet='~/sispop-onion-blockchain-explorer/build/lokblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Sispop emission 

Obtaining current Sispop emission amount is not straight forward. Thus, by default it is 
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.sispop/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.sispop/testnet/lmdb/emission_amount.txt` or `~/.sispop/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.sispop/lmdb/emission_amount.txt`
...
```
Sispop emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Sispop daemon's  `print_coinbase_tx_sum` command. 
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_sispop",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "sispop_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Sispop.
...
Return raw json tx data, as represented in Sispop.
